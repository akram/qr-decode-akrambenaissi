...
public class QRCodeDecoder {
	...
	private static DebugCanvas canvas = new DebugCanvasAdapter();;
	...
	public byte[] decode(QRCodeImage qrCodeImage)
		...
		for (int i = 0; i < results.size(); i++) {
			DecodeResult result = (DecodeResult) results.get(i);
		...
		return ((DecodeResult) results.get(minErrorIndex)).getDecodedBytes();
	}
	...
	Point[] getAdjustPoints() {
		...
		// Point(-2,-2)};
		List adjustPoints = new ArrayList();
		for (int d = 0; d < 4; d++)
		...
		for (int i = 0; i < adjusts.length; i++) {
			adjusts[i] = (Point) adjustPoints.get(i);
		}
		return adjusts;
	}
